...
Return a valid course order or empty when a dependency cycle exists.

## Example

prerequisite 0 before 1 → [0,1].
...
- Signals in the question: topological sort constraints.
...
- Key invariant: Only zero-indegree vertices enter the result, and every emitted edge decrements its target exactly once.
...
- Invariant: Only zero-indegree vertices enter the result, and every emitted edge decrements its target exactly once.
