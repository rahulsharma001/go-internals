...
company_focus:
  ...
  - uber
priority: P0
pattern: topological-sort-characters
difficulty:
...
Infer one valid character order from a sorted list of words, or report contradiction.

## Example

[za,zb,ca,cb] admits z before c and a before b.
...
- Signals in the question: topological sort characters constraints.
...
- Key invariant: Only the first differing character creates an ordering edge; every seen character is a vertex.
...
- Invariant: Only the first differing character creates an ordering edge; every seen character is a vertex.
- Complexity: O(total characters + edges) time.
