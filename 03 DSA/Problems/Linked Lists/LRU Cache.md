...
Support constant-time lookup/update while evicting the least recently used key.

## Example

capacity 2: put a,b; get a; put c → b is evicted.
...
- Signals in the question: hash map doubly linked list constraints and repeated local decisions.
...
- Key invariant: The list orders exactly the map entries from most to least recent.
...
- Invariant: The list orders exactly the map entries from most to least recent.
- Complexity: O(1) expected per operation, O(capacity) space.
