...
difficulty:
leetcode_url: https://leetcode.com/problems/remove-nth-node-from-end/
status: not-started
first_attempt_date:
last_attempt_date:
next_review_date:
...
Remove the node n positions from the end in one pass.

## Example

1→2→3, n 2 becomes 1→3.
...
- Signals in the question: two pointer gap constraints and repeated local decisions.
...
- Key invariant: Fast stays n edges ahead of slow relative to a dummy node.
...
- Invariant: Fast stays n edges ahead of slow relative to a dummy node.
