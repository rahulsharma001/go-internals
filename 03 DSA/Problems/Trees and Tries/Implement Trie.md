...
pattern: trie
difficulty:
leetcode_url: https://leetcode.com/problems/implement-trie/
status: not-started
first_attempt_date:
last_attempt_date:
next_review_date:
...
Support inserting words, exact lookup, and prefix lookup.

## Example

insert car; search car true; search ca false; prefix ca true.
...
- Signals in the question: trie constraints.
...
- Key invariant: A node represents exactly one consumed prefix; terminal distinguishes words from prefixes.
...
- Invariant: A node represents exactly one consumed prefix; terminal distinguishes words from prefixes.
