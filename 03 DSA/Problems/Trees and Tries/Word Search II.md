...
Find dictionary words that can be traced through adjacent board cells without reusing a cell.

## Example

board [a,b], words [ab,aa] → [ab].
...
- Signals in the question: trie backtracking constraints.
...
- Key invariant: The trie node matches the current path prefix and the visited state excludes path cells.
...
- Invariant: The trie node matches the current path prefix and the visited state excludes path cells.
- Complexity: O(board search bounded by trie).
