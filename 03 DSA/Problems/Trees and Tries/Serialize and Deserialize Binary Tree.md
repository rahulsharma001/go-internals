...
Convert a tree to a string and reconstruct the identical shape and values.

## Example

root 1 with only right child 2 round-trips with its missing-left marker.
...
- Signals in the question: tree encoding constraints.
...
- Key invariant: The token stream includes null positions so structure is unambiguous.
...
- Invariant: The token stream includes null positions so structure is unambiguous.
