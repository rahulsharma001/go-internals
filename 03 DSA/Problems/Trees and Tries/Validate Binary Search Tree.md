...
Decide whether every tree node obeys strict BST ordering.

## Example

root 4, left 2, right 5 → true.
...
- Signals in the question: tree bounds constraints.
...
- Key invariant: Each node value must lie strictly inside bounds inherited from all ancestors, not only its parent.
...
- Invariant: Each node value must lie strictly inside bounds inherited from all ancestors, not only its parent.
