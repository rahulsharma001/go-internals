...
Find a target in a distinct sorted array rotated at an unknown pivot.

## Example

[6,8,1,3,4], target 3 → index 3.
...
- Signals in the question: rotated binary search constraints and repeated local decisions.
...
- Key invariant: At least one half is sorted; range checks identify which half can contain the target.
...
- Invariant: At least one half is sorted; range checks identify which half can contain the target.
