...
Decide whether values can be split into two subsets with equal total.

## Example

[1,2,3] → true using {3} and {1,2}.
...
- Signals in the question: subset sum dp constraints.
...
- Key invariant: Reachable[s] means some processed subset sums exactly to s; update sums backward per item.
...
- Invariant: Reachable[s] means some processed subset sums exactly to s; update sums backward per item.
