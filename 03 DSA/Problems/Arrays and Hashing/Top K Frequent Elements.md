...
Return the k values occurring most often.
...
- Signals in the question: frequency map buckets constraints and repeated local decisions.
...
- Key invariant: Every result comes from the highest remaining frequency bucket.
...
- Invariant: Every result comes from the highest remaining frequency bucket.
- Complexity: O(n) time and O(n) space with buckets.
