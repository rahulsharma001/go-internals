...
Decide whether a length-at-least-two range has a sum divisible by k.

## Example

[5,0,0], k=3 → true from [0,0].
...
- Signals in the question: prefix remainder constraints and repeated local decisions.
...
- Key invariant: For each remainder, retain its earliest index so the valid distance is maximized.
...
- Invariant: For each remainder, retain its earliest index so the valid distance is maximized.
