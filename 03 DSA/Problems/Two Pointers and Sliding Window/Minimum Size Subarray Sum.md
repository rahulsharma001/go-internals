...
company_focus:
  ...
  - uber
priority: P1
pattern: positive-sliding-window
difficulty:
...
Find the shortest positive-number range whose sum reaches a target.
...
- Signals in the question: positive sliding window constraints and repeated local decisions.
...
- Key invariant: With positive values, removing from the left strictly decreases the sum.
...
- Invariant: With positive values, removing from the left strictly decreases the sum.
