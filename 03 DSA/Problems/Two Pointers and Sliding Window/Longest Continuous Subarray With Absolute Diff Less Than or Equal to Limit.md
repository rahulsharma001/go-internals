...
company_focus:
  ...
  - uber
priority: P0
pattern: monotonic-deques-window
difficulty:
...
Find the longest range whose maximum minus minimum stays within a limit.

## Example

[4,2,3,6], limit 2 → length 3.
...
- Signals in the question: monotonic deques window constraints and repeated local decisions.
...
- Key invariant: Deque fronts are the current window maximum and minimum; all stored indices are in-window.
...
- Invariant: Deque fronts are the current window maximum and minimum; all stored indices are in-window.
